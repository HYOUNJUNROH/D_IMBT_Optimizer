...
void CUniButton::DrawButtonCaption(HDC hDC, CRect * pRect, BOOL bEnabled, BOOL bSunken) 
{
	...
	// Multiline: Get the number of line
	int curPos = 0;
	int num = 1;
	while( (curPos = text.Find('\n', curPos)) > 0 )
	{
		num++;
		curPos++;
	}
	...
	CString resToken;
	double nLineSpace = tm.tmHeight/2.;
	curPos = 0;

	p.y -= tm.tmHeight*(int(num/2));

	if (bEnabled) {

		for(int i = 0; i < num; i++)
		{
			int nFindAt = text.Find(L"\n", curPos);

			if( nFindAt == -1 )
				nFindAt = text.GetLength();

			resToken = text.Mid(curPos, nFindAt-curPos);
			SetTextColor(hDC, GetSysColor(COLOR_BTNTEXT));
			SetTextAlign(hDC, TA_CENTER | TA_BOTTOM);
			TextOut(hDC, p.x, p.y, resToken, resToken.GetLength());
...
BOOL CUniButton::SubclassDlgItem(UINT nID, CWnd* pParent)
{
	BOOL ret = CButton::SubclassDlgItem(nID, pParent);
	...
	if( ret )
	{
		...
		m_hRgn = CreateRoundRectRgn(0, 0, rect.Width(), rect.Height(), 5, 5);
...
BOOL CUniButton::SubclassDlgItem(UINT nID, CWnd* pParent, UINT nBorder, COLORREF nColor, COLORREF sColor, COLORREF hColor, COLORREF dColor)
{
	m_nBorder = 1;
	m_nColor = nColor;
	m_sColor = sColor;
...
void CUniButton::SetWindowText(LPCTSTR lpszString)
{
...
void CUniButton::MoveWindow(LPCRECT lpRect, BOOL bRepaint /* = TRUE */)
{
...
void CUniButton::OnShowWindow(BOOL bShow, UINT nStatus)
{
	CButton::OnShowWindow(bShow, nStatus);
}

BOOL CUniButton::PreTranslateMessage(MSG* pMsg)
{
