...
for jj=1:length(angle_pos)
    for ii=1:size(source_pos,1)
        ...
        if ii==1
            theta_x2= (atan((source_pos(ii,2)-source_pos(ii+1,2))/(source_pos(ii,3)-source_pos(ii+1,3)+0.0001))*180/pi);
            theta_y2= (-atan((source_pos(ii,1)-source_pos(ii+1,1))/(source_pos(ii,3)-source_pos(ii+1,3)+0.0001))*180/pi);
            theta_z2= angle_pos(jj);
        else
            theta_x2= (atan((source_pos(ii-1,2)-source_pos(ii,2))/(source_pos(ii-1,3)-source_pos(ii,3)+0.0001))*180/pi);
            theta_y2= (-atan((source_pos(ii-1,1)-source_pos(ii,1))/(source_pos(ii-1,3)-source_pos(ii,3)+0.0001))*180/pi);
            theta_z2= angle_pos(jj);
        end
        ...
        %theta = [-theta_x2*pi/180 -theta_y2*pi/180 -theta_z2*pi/180];
