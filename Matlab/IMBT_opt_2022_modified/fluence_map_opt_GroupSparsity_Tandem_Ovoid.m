function [ x wb ]= fluence_map_opt_GroupSparsity_Tandem_Ovoid(A_PTV,d_PTV,A_OAR,d_OAR, opts); 
...
seg= floor((size(A_PTV,2)-10)/opts.Angles);
trunc= opts.trunc;
...
for b=1:opts.Angles+10
    if b<=opts.Angles
        A_b= A(:,(b-1)*seg+1:b*seg);
        wb(b)= c*(normest(A_b))^0.25; %0.25;
    else
        A_b= A(:,opts.Angles*seg+(b-opts.Angles));
        wb(b)= mean(wb(1:opts.Angles)); %c*(normest(A_b))^0.25; %0.25;
    end
end
...
%     x= tempx; %max(tempx,0);
    ...
    for b=1:opts.Angles+10;
        ...
        if b<=opts.Angles
            ...
            zb= z(opts.Angles*seg+(b-opts.Angles),1);
            lambda_b= lambda(opts.Angles*seg+(b-opts.Angles),1);
        end
...
%             x(36*seg+(b-36),1)= tempx;
            x(opts.Angles*seg+(b-opts.Angles),1)= u;
        end
    end   
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    sz=size(x);
    tempx= x(1:(opts.Angles*seg),1);
    x2= convert_order_x(tempx, opts.Angles);
    x2(trunc:(opts.Angles*seg),1)=0;
    tempx2= convert_reverse_order_x(x2, opts.Angles);
    x(1:(opts.Angles*seg),1)= tempx2;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
