function [PTV, bladder, rectum, bowel, sigmoid, body, ind_bodyd] = create_dosematrix_pure_matlab(PatientID, CTData, StructData, Catheter)
...
clearvars -except PatientID CTData StructData Catheter
close all;
...
%{
for ii=1:length(tandem)-1
    theta_x2(ii)= (atan((tandem(ii,2)-tandem(ii+1,2))/(tandem(ii,3)-tandem(ii+1,3)))*180/pi);
    theta_y2(ii)= (-atan((tandem(ii,1)-tandem(ii+1,1))/(tandem(ii,3)-tandem(ii+1,3)))*180/pi);
end
%}
...
diff_tandem= diff(tandem);
dist= sqrt(diff_tandem(:,1).^2+diff_tandem(:,2).^2+diff_tandem(:,3).^2)
dist_tot= dist(1); %sum(dist);

pE(1)= tandem(1,1)+ 3*(tandem(2,1)-tandem(1,1));
pE(2)= tandem(1,2)+ 3*(tandem(2,2)-tandem(1,2));
pE(3)= tandem(1,3)+ 3*(tandem(2,3)-tandem(1,3));

pA(1)= tandem(1,1); pA(2)= tandem(1,2); pA(3)= tandem(1,3);
...
for jj=1:length(angle_pos)
    for ii=1:size(source_pos,1)
        ...
        %Arot1= rotate3D(A,theta_x2,theta_y2,theta_z2,vA);
        %rotate3D를 아래로 변경
